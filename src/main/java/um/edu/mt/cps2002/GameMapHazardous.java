...
public class GameMapHazardous extends GameMap{
    ...
    private Colour generateTile(){
        ...
        if(mapProp<0.35 && Math.random()<=0.35)
            ret = Colour.BLUE;
        else
            ret = Colour.GREEN;
    ...
    /*! Used when map has been filled but fraction of blue tiles under 0.25*/
    private Colour generateTile_moreblue(){
        Colour ret = Colour.BLUE;
        double mapProp = mapProportion();
    ...
    public void generate()
    {
        ...
        while(mapProportion()<0.25){
            for(int i=0;i<size;i++) {
                for (int j = 0; j < size; j++) {
                    if(tileColours[i][j]==Colour.GREEN)
                        tileColours[i][j] = generateTile_moreblue();
                }
            }
        }
